...
export const addOrderItem = async (req, res) => {
  ...
  if (!order_id || !product_id || !quantity || !price) {
    return res.status(400).json({ success: false, message: "All fields (order_id, product_id, quantity, price) are required" });
  }
  if (quantity <= 0) {
  ...
  try {
    ...
    res.status(201).json({ success: true, orderItem: result.rows[0] });
  } catch (error) {
    console.error("Add order item error:", error);
    res.status(500).json({ success: false, message: "Error adding order item", error: error.message });
  }
};
...
export const getOrderItems = async (req, res) => {
  ...
  try {
    ...
    res.json({ success: true, count: result.rowCount, orderItems: result.rows });
  } catch (error) {
...
export const updateOrderItem = async (req, res) => {
  ...
  try {
    ...
    if (itemCheck.rows.length === 0) {
      return res.status(404).json({ success: false, message: `Order item with ID ${itemId} not found` });
    }

    const orderStatus = itemCheck.rows[0].order_status;
    if (["Shipped", "Delivered", "Cancelled"].includes(orderStatus)) {
      return res.status(400).json({ success: false, message: `Cannot modify items in ${orderStatus} orders` });
    }
    ...
    res.json({ success: true, message: "Order item updated successfully", orderItem: result.rows[0] });
  } catch (error) {
...
export const removeOrderItem = async (req, res) => {
  ...
  try {
    ...
    const orderStatus = itemCheck.rows[0].order_status;
    if (["Shipped", "Delivered", "Cancelled"].includes(orderStatus)) {
      return res.status(400).json({ success: false, message: `Cannot remove items from ${orderStatus} orders` });
    }

    const result = await pool.query(`DELETE FROM order_items WHERE id = $1 RETURNING *`, [itemId]);

    res.json({ success: true, message: "Order item removed successfully", orderItem: result.rows[0] });
  } catch (error) {
    console.error(`Remove order item error (ID: ${itemId}):`, error);
    res.status(500).json({ success: false, message: "Error removing order item", error: error.message });
  }
};
...
export const getOrderItemById = async (req, res) => {
  ...
  try {
    ...
    res.json({ success: true, orderItem: result.rows[0] });
  } catch (error) {
